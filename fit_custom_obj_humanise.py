...
if __name__ == "__main__":
    ...
    with open(args.file_name, 'rb') as f:
        cluster_contact_points = np.load(f)
        cluster_contact_points = cluster_contact_points.squeeze()
        print(len(cluster_contact_points))
    contact_pcd = o3d.geometry.PointCloud()
    ...
    for i, obj_c in enumerate(clusters_classes):
        ...
        tmp_path = os.path.join(output_dir, sequence_name, "fit_best_obj", obj_class_str)
        if os.path.exists(tmp_path):
            start_idx = len(os.listdir(tmp_path))
        else:
            start_idx = 0
        obj_idx += start_idx
