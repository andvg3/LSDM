...
def train():
    ...
    for mask, given_objs, given_cats, target_obj, target_cat, y in tqdm(train_data_loader):
        ...
        boxes = {}
        # Create contact labels for boxes
        boxes['contact_labels'] = torch.zeros((bs, num_obj, 1))
        boxes['contact_labels'][:, 0] = 1
        boxes['contact_labels'] = boxes['contact_labels'].to(device)
...
if __name__ == '__main__':
    ...
    config_path = os.path.join("atiss", "config", "bedrooms_eval_mime_config.yaml")
